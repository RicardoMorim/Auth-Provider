...
public class RoleManagementController<U extends AuthUser<ID, R>, R extends Role, ID> {
    ...
    private String validateUsername(String usernameStr) {
        try {
            ...
            log.warn("Username validation failed for '{}': {}", LogSanitizer.sanitize(usernameStr), e.getMessage());
            throw e;
        }
    }
    ...
    private ID getUserIdByUsername(String username) {
        try {
            ...
            if (user == null) {
                throw new ResourceNotFoundException("User not found: " + LogSanitizer.sanitize(username));
            }
            return user.getId();
        } catch (ResourceNotFoundException e) {
            throw e;
        } catch (Exception e) {
            log.warn("User lookup failed for username: {}", LogSanitizer.sanitize(username), e);
            throw new IllegalArgumentException("User lookup failed: " + LogSanitizer.sanitize(username), e);
        }
    }
    ...
    public ResponseEntity<UserRolesResponse> getUserRoles(
        ...
        try {
            ...
            log.debug("Retrieved roles for user: {}", LogSanitizer.sanitize(validatedUsername));
            return ResponseEntity.ok(response);

        } catch (ResourceNotFoundException e) {
            log.warn("User not found for username: {}: {}", LogSanitizer.sanitize(username), e.getMessage());
            return ResponseEntity.status(404).build();
        } catch (IllegalArgumentException e) {
            log.warn("Invalid request for user roles: {}", LogSanitizer.sanitize(e.getMessage()));
            return ResponseEntity.badRequest().build();
        } catch (SecurityException e) {
            log.warn("Security violation accessing user roles for username: {}", LogSanitizer.sanitize(username));
            return ResponseEntity.status(403).build();
        } catch (Exception e) {
            log.error("Error retrieving user roles for username: {}", LogSanitizer.sanitize(username));
    ...
    public ResponseEntity<Map<String, String>> addRoleToUser(
        ...
        try {
            ...
            log.info("Role '{}' added to user {} with reason: {}",
                    LogSanitizer.sanitize(request.getRoleName()),
                    LogSanitizer.sanitize(validatedUsername),
                    LogSanitizer.sanitize(request.getReason()));
            ...
            log.warn("User or role not found for username: {}: {}", LogSanitizer.sanitize(username), e.getMessage());
            return ResponseEntity.status(404)
                    .body(Map.of("error", e.getMessage()));
        } catch (IllegalArgumentException e) {
            log.warn("Invalid role addition request for username {}: {}", LogSanitizer.sanitize(username), LogSanitizer.sanitize(e.getMessage()));
            return ResponseEntity.badRequest()
                    .body(Map.of("error", e.getMessage()));
        } catch (SecurityException e) {
            log.warn("Security violation adding role to username: {}", LogSanitizer.sanitize(username));
            return ResponseEntity.status(403)
                    .body(Map.of("error", "Insufficient permissions."));
        } catch (Exception e) {
            log.error("Error adding role to user: {}", LogSanitizer.sanitize(username));
    ...
    public ResponseEntity<Map<String, String>> removeRoleFromUser(
        ...
        try {
            ...
            log.warn("Invalid role removal request for username {}: {}", LogSanitizer.sanitize(username), LogSanitizer.sanitize(e.getMessage()));
            return ResponseEntity.badRequest()
                    .body(Map.of("error", e.getMessage()));
        } catch (SecurityException e) {
            log.warn("Security violation removing role from username: {}", LogSanitizer.sanitize(username));
            return ResponseEntity.status(403)
                    .body(Map.of("error", "Insufficient permissions."));
        } catch (Exception e) {
            log.error("Error removing role from user: {}", LogSanitizer.sanitize(username));
            return ResponseEntity.status(500)
    ...
    public ResponseEntity<Map<String, String>> bulkUpdateUserRoles(
        ...
        try {
            ...
            log.info("Bulk role update completed for user {} - Added: {}, Removed: {}",
                    LogSanitizer.sanitize(validatedUsername),
                    LogSanitizer.sanitize(String.join(", ", request.getRolesToAdd())),
                    LogSanitizer.sanitize(String.join(", ", request.getRolesToRemove())));
            ...
            log.warn("Invalid bulk role update request for username {}: {}", LogSanitizer.sanitize(username), LogSanitizer.sanitize(e.getMessage()));
            return ResponseEntity.badRequest()
                    .body(Map.of("error", e.getMessage()));
        } catch (SecurityException e) {
            log.warn("Security violation in bulk role update for username: {}", LogSanitizer.sanitize(username));
            return ResponseEntity.status(403)
                    .body(Map.of("error", "Insufficient permissions."));
        } catch (Exception e) {
            log.error("Error in bulk role update for user: {}", LogSanitizer.sanitize(username));
            return ResponseEntity.status(500)
